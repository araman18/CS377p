...
void *computation(void *threadIdPtr) {
    ...
    int num_edges = graph->size_edges();

    for(int j = threadId; j <= num_edges; j+=numThreads) {
        int destinationNode = graph->get_edge_dst(j);
        int currEdge = j;
        int l = 1, r = num_nodes;
        int srcNode = -1;
        while(l < r) {
            int mid = l + ((r - l) / 2);
            int start = graph->edge_begin(mid), end = graph->edge_end(mid);
            if(currEdge >= start && currEdge < end) {
                srcNode = mid;
                break;
            } else if(currEdge < start) {
                r = mid - 1;
            } else {
                l = mid + 1;
            }
        ...
        if(srcNode == -1) srcNode = l;

        double contribution = damping * graph->get_label(srcNode, CURRENT) / (double) graph->get_out_degree(srcNode);

        graph->set_label(destinationNode, NEXT, graph->get_label(destinationNode, NEXT) + contribution);
